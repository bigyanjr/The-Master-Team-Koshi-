...
export default function AddUpdate() {
  ...
  return (
    <Card>
      <CardHeader title="Post Progress Update" subtitle="Updates progress and adds a during-work proof photo" />
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          ...
          <select required value={form.projectId} onChange={(e) => update('projectId', e.target.value)}
            ...
            {projects.map((p) => (
              <option key={p.id} value={p.id}>{p.title} ({p.progressPercent}%)</option>
            ))}
          </select>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input required type="number" min="0" max="100" value={form.progressAfter} onChange={(e) => update('progressAfter', e.target.value)}
              className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/30" placeholder="0–100" />
          </div>
