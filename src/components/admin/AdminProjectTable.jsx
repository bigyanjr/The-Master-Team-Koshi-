  ...
  return (
    <div className="p-4 rounded-xl border border-slate-200 bg-white space-y-3">
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className="font-bold text-brand-950 leading-snug">{project.title}</p>
          <p className="text-xs text-slate-500 mt-0.5">Ward {ward?.number} · {project.category}</p>
        </div>
        <RiskLevelBadge level={risk.label} />
      </div>
      <div className="flex flex-wrap gap-2 text-xs">
        <StatusBadge status={project.status} />
        <span className="font-bold text-slate-700">Trust {trust}</span>
        <span className="text-slate-500">{formatCurrency(project.allocatedBudget)}</span>
      </div>
        ...
        <table className="w-full text-sm min-w-[720px]">
          <thead>
            <tr className="border-b border-slate-100 text-left bg-slate-50/80">
              <th className="px-5 py-3 font-semibold text-slate-500 text-xs uppercase">Project</th>
              <th className="px-3 py-3 font-semibold text-slate-500 text-xs uppercase">Ward</th>
              <th className="px-3 py-3 font-semibold text-slate-500 text-xs uppercase">Budget</th>
              <th className="px-3 py-3 font-semibold text-slate-500 text-xs uppercase w-28">Progress</th>
              <th className="px-3 py-3 font-semibold text-slate-500 text-xs uppercase">Status</th>
              <th className="px-3 py-3 font-semibold text-slate-500 text-xs uppercase">Risk</th>
              <th className="px-5 py-3 font-semibold text-slate-500 text-xs uppercase">Actions</th>
            </tr>
          </thead>
          <tbody>
            {projects.map((project) => {
              ...
              return (
                ...
                >
                  <td className="px-5 py-4">
                    <p className="font-semibold text-brand-950 line-clamp-1">{project.title}</p>
                  ...
                  <td className="px-3 py-4">
                    ...
                    <span className="text-[10px] text-slate-400">{project.progressPercent}%</span>
                  </td>
